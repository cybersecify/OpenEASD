...
| `apps/tldsquatting/` | 1 | Brand Threat | Yes | TLD-squatting / lookalike domain detection (**supersedes the former `typosquat` tool** — broader TLD coverage). Generates lookalike candidates algorithmically: **TLD permutation across ~900 registrable TLDs** (bundled `tlds.txt`, emitted first so the high-precision "exact name, other TLD" signal survives the candidate cap) plus character-level typos (**off by default** — `TLDSQUATTING_INCLUDE_TYPOS=True` to enable; TLD cybersquatting is the focus). Checks which are registered via public DNS (extended for registered candidates to the full posture: A/AAAA/MX/NS-targets/CNAME/TXT/SPF/DMARC/CAA/DNSSEC + a capped, fail-graceful homepage fetch for a login form / brand mention), then rates each via a **ported risk + threat scoring model** (`scoring.py`): `calculate_risk_score` (registration recency + infra + suspicious DNS combos + security config + HTTPS + NS-provider tier) → `calculate_threat_score` (adds login-form/brand/parked). The **threat band maps to severity** (`PRE-EXISTING→info, LOW→low, MEDIUM→medium, HIGH→high, CRITICAL→critical`), with numeric scores + levels stored in `extra`. **RDAP registration-age** (`rdap.org`, `TLDSQUATTING_RDAP_AGE`) feeds the model's dominant **PRE-EXISTING** rule: a lookalike registered before the target scores 0.0 → **info** (it can't be impersonating a domain that didn't exist yet — e.g. `amnic.net` 1994 vs `amnic.com` 1997). `check_type="lookalike_domain"` (consumed by `asn_cluster`, which treats `high`/`critical` as weaponized). Passive w.r.t. the target (contacts only the lookalike domains + RDAP, never yours), no key, fail-graceful. |
...
| `tests/unit/test_tldsquatting.py` | 58 | candidate generation (all 8 techniques, uniqueness, no-original, www-strip, cap/truncation-logged), passive DNS registration check (A/MX/NS + full posture, NXDOMAIN + timeout never raise), weaponization homepage probe (login form + brand mention flagged, fetch failure graceful), RDAP registration-age (parse/fail-graceful/predates), **ported scoring model** (PRE-EXISTING when predates/apex, email-infra combo → critical, NS-tier classification, threat content signals, bad-input never raises), analyzer severity mapped from threat band (predates → info, login-form+infra → high, fully-weaponized-recent → critical, enterprise-NS lowers, parked → low, scores stored in extra), scanner (saves + never-raises), concurrent collect (order-preserving, all-registered-checked, fetch cap) |
