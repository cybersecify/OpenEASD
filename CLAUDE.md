...
- **Publish triggers:** every push to `main` (`:latest` tag) and `v*` git tags (`:vX.Y.Z` tag, from `type=ref,event=tag`); a `:vX.Y` major.minor convenience tag is added out-of-band via `docker buildx imagetools create` (see release steps below)
