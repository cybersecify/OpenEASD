"""API views for Insights — pre-computed summaries + on-the-fly asset metrics."""
...
def api_insights(request):
    ...
    summaries = list(reversed(summaries))  # oldest-first for trend charts
    ...
    # ----- Top vulnerable services (group by service+version in Python) -----
    # Avoid Max() on JSONField — not reliable with Django/SQLite
