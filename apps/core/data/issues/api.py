...
Reads and triages the ``Issue`` register. A status change here **persists across
scans** (unlike per-scan ``Finding.status``) — dismiss a false positive once and
it stays dismissed. Split out of the findings app (D-017).
"""
...
from apps.core.data.findings.models import STATUS_CHOICES
