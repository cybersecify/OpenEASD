...
in the database and executed dynamically by apps.core.engine.workflows.runner.
...
def _finalize_session(session):
    ...
    if session.scan_type == "subscan":
        ...
        from apps.core.console.ai.hooks import maybe_continue_agent
    ...
    # summary.
    from apps.core.console.ai.hooks import run_ai_post_scan
...
def _seed_apex_into_assets(session) -> None:
    ...
    from apps.core.data.assets.models import IPAddress, Subdomain
    from django.utils import timezone as django_tz
...
def phase_groups_for_session(session_id: int) -> list:
    ...
    from apps.core.engine.workflows.runner import resolve_phase_groups
...
def run_phase_group_for_session(session_id: int, group: list) -> None:
    ...
    from apps.core.engine.workflows.models import WorkflowRun, WorkflowStepResult
