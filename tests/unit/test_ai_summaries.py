"""Unit tests for apps/core/console/ai/summaries.py."""

import pytest
from unittest.mock import patch

from apps.core.console.ai.models import AISummary, AITriage
from apps.core.console.ai.summaries import run_summaries
...
class TestRunSummaries:
    def test_both_kinds_written(self):
        sess = _session()
        with patch("apps.core.console.ai.summaries.client.chat_json",
    ...
    def test_failure_leaves_absent(self):
        sess = _session()
        with patch("apps.core.console.ai.summaries.client.chat_json", return_value=None):
            run_summaries(sess)
    ...
    def test_empty_text_treated_as_failure(self):
        sess = _session()
        with patch("apps.core.console.ai.summaries.client.chat_json", return_value={"text": "   "}):
            run_summaries(sess)
        assert AISummary.objects.count() == 0
    ...
    def test_rerun_updates_in_place(self):
        sess = _session()
        with patch("apps.core.console.ai.summaries.client.chat_json", return_value={"text": "v1"}):
            run_summaries(sess)
        with patch("apps.core.console.ai.summaries.client.chat_json", return_value={"text": "v2"}):
            run_summaries(sess)
