...
class TestRunScanCommand:
    def test_starts_scan_for_domain(self):
        ...
        with patch("apps.core.engine.scans.management.commands.run_scan.create_scan_session", return_value=sess) as mock_create, \
             patch("apps.core.engine.scans.management.commands.run_scan.run_scan") as mock_run:
            out = StringIO()
    ...
    def test_prints_error_when_scan_already_active(self):
        with patch("apps.core.engine.scans.management.commands.run_scan.create_scan_session", return_value=None):
            out = StringIO()
            call_command("run_scan", "--domain", "example.com", stdout=out)
    ...
    def test_prints_final_status(self):
        ...
        with patch("apps.core.engine.scans.management.commands.run_scan.create_scan_session", return_value=sess), \
             patch("apps.core.engine.scans.management.commands.run_scan.run_scan"):
            out = StringIO()
            call_command("run_scan", "--domain", "example.com", stdout=out)
        output = out.getvalue()
...
class TestRunDailyScanCommand:
    def test_calls_daily_scan(self):
        with patch("apps.core.engine.scans.management.commands.run_daily_scan.daily_scan") as mock_daily:
    ...
    def test_prints_done_on_success(self):
        with patch("apps.core.engine.scans.management.commands.run_daily_scan.daily_scan", return_value=None):
            out = StringIO()
            call_command("run_daily_scan", stdout=out)
...
class TestRunWeeklyScanCommand:
    def test_calls_daily_scan(self):
        with patch("apps.core.engine.scans.management.commands.run_weekly_scan.daily_scan") as mock_scan:
            out = StringIO()
            call_command("run_weekly_scan", stdout=out)
        mock_scan.assert_called_once()

    def test_prints_done_on_success(self):
        with patch("apps.core.engine.scans.management.commands.run_weekly_scan.daily_scan"):
            out = StringIO()
            call_command("run_weekly_scan", stdout=out)
        assert "Done" in out.getvalue()
...
class TestBackfillInsightsCommand:
    def test_backfills_completed_scans_without_summary(self):
        ...
        with patch("apps.core.console.insights.management.commands.backfill_insights.build_insights") as mock_build:
            out = StringIO()
    ...
    def test_prints_summary_count(self):
        ...
        with patch("apps.core.console.insights.management.commands.backfill_insights.build_insights"):
            out = StringIO()
            call_command("backfill_insights", stdout=out)
