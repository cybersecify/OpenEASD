...
class TestCreateScanSession:
    ...
    def test_returns_none_on_database_error_when_scan_active(self, db):
        ...
        with patch("apps.core.engine.scans.pipeline.ScanSession.objects.select_for_update", side_effect=DatabaseError("lock")):
    ...
    def test_retries_and_creates_session_on_transient_lock(self, db):
        ...
        call_count = {"n": 0}
        original = __import__("apps.core.engine.scans.models", fromlist=["ScanSession"]).ScanSession.objects.select_for_update
        ...
        with patch("apps.core.engine.scans.pipeline.ScanSession.objects.select_for_update", side_effect=patched_select_for_update):
...
class TestParseSchedule:
    ...
    def test_recurring_maps_fields(self):
        from apps.core.engine.scans.api import _parse_schedule
...
class TestDetectDeltas:
    def test_new_findings_create_new_deltas(self, db):
        ...
        from apps.core.engine.scans.pipeline import _detect_deltas
...
class TestFinalizeSubscan:
    def test_finalize_skips_deltas_insights_alerts_for_subscan(self, db):
        ...
        with patch("apps.core.engine.scans.pipeline._detect_deltas") as m_delta, \
             patch("apps.core.console.insights.builder.build_insights") as m_insights, \
             patch("apps.core.engine.scans.pipeline._dispatch_alerts") as m_alerts:
...
class TestRunScanPendingGuard:
    def test_cancelled_session_is_not_resurrected(self, db):
        ...
        with patch("apps.core.engine.scans.pipeline._run_via_workflow") as m_run:
            run_scan(s.id)
    ...
    def test_pending_session_runs(self, db):
        ...
        with patch("apps.core.engine.scans.pipeline._run_via_workflow") as m_run, \
             patch("apps.core.engine.scans.pipeline._seed_apex_into_assets"), \
             patch("apps.core.engine.scans.pipeline._finalize_session"):
            run_scan(s.id)
...
class TestScanDurationLabel:
    ...
    def test_completed_scan_returns_took(self):
        from apps.core.engine.scans.templatetags.scan_tags import scan_duration_label
    ...
    def test_running_scan_returns_running(self):
        ...
        with patch("apps.core.engine.scans.templatetags.scan_tags.timezone") as mock_tz:
